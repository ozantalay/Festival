...
export default function PosterSection({ date, festivalData }) {
  /*Challenge
    ...
    const sortedGroups = festivalData.filter(group=>group.date===date)
    .sort((a,b)=>a.importance-b.importance)

    const getClassName = (importance) => {
      if (importance === 1) return 'giant';
      if (importance === 2) return 'big';
  ...
  return (
    <div className='lineup-container'>
      ...
      {sortedGroups.map((group, index) => (
        <p key={index} className={getClassName(group.importance)}>{group.name} </p>
    ))}
